...
import {Database, Eye, GitBranch, Star} from 'lucide-react';
...
function Repositories() {
    ...
    <Database/>
        ...
        <p className="text-gray-500 mb-8">Here are some of the repositories from my <a
            href="https://github.com/ofluffydev" className="text-blue-300 hover:text-blue-600 underline">GitHub</a>
            .</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayedRepositories.map((repo) => <div key={repo.id}
                                                      className="hover:shadow-purple-400 hover:shadow-2xl bg-gray-800 shadow rounded-lg p-4 hover:shadow-md transition-shadow duration-300">
        ...
        {!showAll && repositories.length > 10 && <div className="text-center mt-6">
            <Button className={'bg-purple-800 hover:bg-purple-300 text-center'} onClick={() => {
                ...
                Show More (Warning, there is a lot...)
            </Button>
        </div>}
    </div>;
}
