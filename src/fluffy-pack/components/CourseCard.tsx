...
const CourseCard: FC<CourseCardProps> = ({image, title, description, disabled = false, url}: CourseCardProps) => {
    ...
    const linkClasses = `
        ...
        ${disabled ? 'bg-gray-600 cursor-not-allowed' : 'bg-purple-800 hover:bg-purple-600 transition-colors duration-300'}
        text-white
    `;

    return (<InView threshold={0.1}>
            {({inView, ref}) => (<div
                    ref={ref}
                    className={`
                        bg-gray-800 rounded-lg shadow-md overflow-hidden
                        transition-all duration-300 ease-in-out
                        hover:shadow-purple-400 hover:-translate-y-2
                ...
                >
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                        ...
                        {disabled ? (<span className={linkClasses}>
                                Learn More
                            </span>) : (<a
                                href={url}
